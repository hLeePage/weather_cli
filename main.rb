...
# Alerts
response = HTTParty.get("http://api.wunderground.com/api/#{KEY}/alerts/q/#{zip}.json")
alerts = response['alerts']
# lzy bstrd
puts "There are no alerts for your area."


# Current conditions
response = HTTParty.get("http://api.wunderground.com/api/#{KEY}/geolookup/conditions/q/#{zip}.json")
location = response['location']['city']
temp_f = response['current_observation']['temp_f']
puts "Current temperature in #{location} is: #{temp_f} degrees Farenheit.\n"


# Sunrise/sunset
response = HTTParty.get("http://api.wunderground.com/api/#{KEY}/astronomy/q/#{zip}.json")
...
# 10 Day Forecast
response = HTTParty.get("http://api.wunderground.com/api/#{KEY}/forecast10day/q/#{zip}.json")
#binding.pry
10.times do |i|
...
puts "Current hurricanes: #{hurricanes}."
